...
/**
 * Intent UI — chart (instalado con `npx shadcn add https://intentui.com/r/radar-chart`).
 *
 * Adaptado a Zenovi: sus tokens de color se tradujeron a los del sistema. No se agregaron
 * los tokens de Intent UI al CSS global porque uno choca: para Intent UI `muted` es un gris
 * de fondo y en Zenovi es el gris de texto. Las series usan `--color-series-*`, validadas
 * para daltonismo.
 */
...
const CHART_COLORS = {
  'chart-1': 'var(--color-series-1)',
  'chart-2': 'var(--color-series-2)',
  'chart-3': 'var(--color-series-3)',
  'chart-4': 'var(--color-series-4)',
  'chart-5': 'var(--color-graphite)',
} as const
    ...
    <ChartContext value={value}>
      <div
        ...
        className={cn(
          ...
          "[&_.recharts-cartesian-axis-tick_text]:fill-graphite [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-mist [&_.recharts-curve.recharts-tooltip-cursor]:stroke-mist [&_.recharts-layer]:outline-hidden [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-mist [&_.recharts-radial-bar-background-sector]:fill-control [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-control [&_.recharts-reference-line_[stroke='#ccc']]:stroke-mist [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-sector]:outline-hidden [&_.recharts-surface]:outline-hidden [&_.recharts-surface_.recharts-text.recharts-cartesian-axis-tick-value]:*:fill-graphite",
          "[&_.recharts-dot[fill='#fff']]:fill-(--line-color)",
          '[&_.recharts-active-dot>.recharts-dot]:stroke-ink/10',
...
const XAxis = ({
  ...
  return (
    <XAxisPrimitive
      className={cn('text-graphite text-xs **:[text]:fill-graphite', className)}
...
const CartesianGrid = ({ className, ...props }: CartesianGridPrimitiveProps) => {
  ...
  return (
    <CartesianGridPrimitive
      className={cn('stroke-1 stroke-control', className)}
...
const ChartTooltipContent = <TValue extends ValueType, TName extends NameType>({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'grid min-w-48 items-start rounded-lg bg-paper/90 p-3 py-2 text-ink text-xs ring ring-current/10 backdrop-blur-lg',
        className
      )}
    >
      {!hideLabel && (
        <>
          ...
          {labelSeparator && <span aria-hidden className="mt-2 mb-3 block h-px w-full bg-paper/10" />}
        </>
      )}
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={key}
              className={cn(
                'flex w-full flex-wrap items-stretch gap-2 *:[svg]:text-graphite',
                ...
                <>
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-graphite">{itemConfig?.label || item.name}</span>
                    </div>
      ...
      {payload.map((item: LegendPayload) => {
        ...
        return (
          <ToggleButton
            ...
            id={key}
            className={cn(
              'flex items-center gap-2 rounded-sm px-2 py-1 text-graphite *:[svg]:-mx-0.5 *:[svg]:size-2.5 *:[svg]:shrink-0 *:[svg]:text-graphite',
              'selected:bg-control/70 selected:text-ink',
              'hover:bg-control/70 hover:text-ink'
            )}
