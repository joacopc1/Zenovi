...
- [ ] Detectar un acceso revocado durante la sincronización: si Instagram rechaza el token, la sincronización lo trata como falla pasajera (`account_resolved`) y nunca ofrece volver a autorizar, así que la cuenta queda trabada con un error genérico. El callback de borrado cubre el caso normal, pero la sincronización tiene que reconocer el error de autorización de Meta y pasar la conexión a `action_required`.
