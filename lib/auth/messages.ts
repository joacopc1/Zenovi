export const SIGN_IN_FAILURE_MESSAGE =
  "No pudimos iniciar sesión con esos datos.";

export const SIGN_UP_RESULT_MESSAGE =
  "Si pudimos crear la cuenta con esos datos, vas a recibir un correo para confirmarla.";

export const PASSWORD_RESET_RESULT_MESSAGE =
  "Si existe una cuenta con ese correo, vas a recibir instrucciones para recuperar el acceso.";

export const PASSWORD_UPDATE_FAILURE_MESSAGE =
  "No pudimos actualizar la contraseña. Solicitá un nuevo enlace e intentá otra vez.";

export const PASSWORD_MIN_LENGTH = 8;
